...
(function() {
  ...
  _.filter = function(collection, test) {
    var truthyResult = [];
    for(var i = 0; i < collection.length; i++){
      if (test(collection[i])) truthyResult.push(collection[i]); 
    }
    return truthyResult;
  };
  ...
  _.map = function(collection, iterator) {
    ...
    var mappedResult = [];
    for(var i = 0; i < collection.length; i++){
      mappedResult.push(iterator(collection[i]));
    }
    return mappedResult;
  };
  ...
  _.invoke = function(collection, functionOrKey, args) {
    ...
    _.each(collection, function(value, index, arr) {
      if(typeof(functionOrKey) === 'function'){
        invokeResult.push(functionOrKey.apply(value, args));
      } else {
        invokeResult.push(value[functionOrKey](args));
      }  
    });
    return invokeResult;
  };
